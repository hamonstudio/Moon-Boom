...
@export var tire_turn_speed := 100.0
@export var tire_max_turn_degrees := 30
@export var tire_back_turn_speed := 20.0
@export var tire_back_max_turn_degrees := 5
@export var aim_turn_speed := 30

@onready var SHIP_AIM = %SHIP_AIM_ROTATION
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("click_left"):
		motor_input = 1
		print("acc UP")
	elif event.is_action_released("click_left"):
		motor_input = 0
	...
	#_basic_steering_animation(motor_input)

func _basic_steering_animation(delta: float) -> void:
	#Ship cabin animation on acceleration/break motion
	#SHIP_AIM.position.z = 0 + SHIP_AIM.position.z + motor_input / delta
	pass
...
func _basic_steering_rotation(delta: float) -> void:
	...
	if turn_input:
		...
		deg_to_rad(-tire_back_max_turn_degrees), deg_to_rad(tire_back_max_turn_degrees))
		#Ship cabin animation on move left/right motion
...
func _basic_aim_rotation(delta: float) -> void:
	var aim_input := Input.get_axis("aim_right","aim_left") * aim_turn_speed
	var turn_input_back := Input.get_axis("move_right","move_left") * tire_back_turn_speed
	if aim_input:
		#Ship cabin animation on move left/right motion
		SHIP_AIM.rotation.y = move_toward(SHIP_AIM.rotation.y, SHIP_AIM.rotation.y + aim_input, delta)
		print("AIMING")
	else:
		#Ship cabin animation on move left/right motion
		SHIP_AIM.rotation.y = move_toward(SHIP_AIM.rotation.y, 0, delta)
