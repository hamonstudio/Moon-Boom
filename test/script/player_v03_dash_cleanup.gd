...
func moving(delta: float) -> void:
	...
	if move_force.length() > 0.2 and doDash:
		...
		force_vector =  acc_dash * move_force - dashDirection * steering_speed_base
		print("move_force : ", move_force, " | dashdir : ",dashDirection)
	# Drift movement
	elif move_force.length() > 0.2 and doDrift:
		steering_speed_modified = steering_speed_base * 4
		force_vector = ac * acc_dash * move_force - dashDirection * steering_speed_base
	# Normal movement
...
func steering(delta: float) -> void:
	...
	# Hips and Leg animated by steering direction 
	var steering_influence = ( move_angle.y / 3 )
...
func dashing():
	drifting()
	dashDirection = target_dash.transform.basis.z.normalized()
	print("is dashing")
	canDash = false
	doDash = true
	doDrift = true
